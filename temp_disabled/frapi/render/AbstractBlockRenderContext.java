...
//				quad.lightmap(i, ColorHelper.maxBrightness(quad.lightmap(i), data.lm[i]));
	...
	public RenderType effectiveRenderLayer(@Nullable RenderType quadRenderLayer) {
		return quadRenderLayer == null ? defaultLayer : quadRenderLayer;
	}

	// 1.21.1: emitVanillaBlockQuads uses BakedModel instead of BlockStateModel/BlockModelPart
	public void emitVanillaBlockQuads(BakedModel model, @Nullable BlockState state, Supplier<RandomSource> randomSupplier, Predicate<Direction> cullTest) {
		// In 1.21.1, vanilla quads are emitted via BakedModel.emitBlockQuads (FRAPI)
		// which is handled by the FRAPI mesh system; this method is called from BlockRenderContext
		if (model instanceof net.fabricmc.fabric.api.renderer.v1.model.FabricBakedModel fabricModel) {
			fabricModel.emitBlockQuads(this.getEmitter(), this.blockAndTintGetter, state, this.blockPos, randomSupplier, cullTest);
		}
	}
