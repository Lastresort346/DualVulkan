...
import net.vulkanmod.render.chunk.build.frapi.compat.FoilType;
...
public final class EncodingFormat {
	...
	// 1.21.1: ChunkSectionLayer replaced by TerrainRenderType enum (VulkanMod-owned)
	private static final @Nullable RenderType[] NULLABLE_BLOCK_RENDER_LAYERS;
	static {
		net.vulkanmod.render.vertex.TerrainRenderType[] terrain = net.vulkanmod.render.vertex.TerrainRenderType.VALUES;
		RenderType[] renderTypes = new RenderType[terrain.length];
		for (int i = 0; i < terrain.length; i++) {
			renderTypes[i] = net.vulkanmod.render.vertex.TerrainRenderType.getRenderType(terrain[i]);
		}
		NULLABLE_BLOCK_RENDER_LAYERS = ArrayUtils.add(renderTypes, null);
	}
	...
	private static final @Nullable FoilType[] NULLABLE_GLINTS = ArrayUtils.add(FoilType.values(), null);
	...
	static int renderLayer(int bits, @Nullable RenderType renderLayer) {
		if (renderLayer == null) {
			return (bits & ~RENDER_LAYER_MASK) | (NULL_RENDER_LAYER_INDEX << RENDER_LAYER_BIT_OFFSET);
		}
		// Find index in our NULLABLE_BLOCK_RENDER_LAYERS array
		for (int i = 0; i < NULLABLE_BLOCK_RENDER_LAYERS.length - 1; i++) {
			if (renderLayer == NULLABLE_BLOCK_RENDER_LAYERS[i]) {
				return (bits & ~RENDER_LAYER_MASK) | (i << RENDER_LAYER_BIT_OFFSET);
			}
		}
		// Unknown render layer — use null slot
		return (bits & ~RENDER_LAYER_MASK) | (NULL_RENDER_LAYER_INDEX << RENDER_LAYER_BIT_OFFSET);
	}
