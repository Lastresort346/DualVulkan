...
void main() {
    ...
    if(vertexDistance > 0.8*FogEnd) {
        ...
        //    vec3 upVec = (ModelViewMat * vec4(0.0, 1.0, 0.0, 0.0)).xyz;
        //    float rDotUp = dot(viewVec, upVec);
        ...
        //    color = vertexDistance > 0.7*FogEnd ? fog(color, vertexDistance, FogEnd, fogColor) : color;
