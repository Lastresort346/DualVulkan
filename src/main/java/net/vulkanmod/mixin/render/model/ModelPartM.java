...
public class ModelPartM {
    ...
    private void compile(PoseStack.Pose pose, VertexConsumer vertexConsumer, int i, int j, int color) {
        ...
        for (ModelPart.Cube cube : this.cubes) {
            ...
            net.vulkanmod.render.model.CubeModel.Polygon[] polygons = cubeModel.getPolygons();

            cubeModel.transformVertices(matrix4f);

            for (net.vulkanmod.render.model.CubeModel.Polygon polygon : polygons) {
                matrix3f.transform(this.temp.set(polygon.normal()));
                this.temp.normalize();

                int packedNormal = VertexUtil.packNormal(temp.x(), temp.y(), temp.z());

                net.vulkanmod.render.model.CubeModel.Vertex[] vertices = polygon.vertices();

                for (net.vulkanmod.render.model.CubeModel.Vertex vertex : vertices) {
