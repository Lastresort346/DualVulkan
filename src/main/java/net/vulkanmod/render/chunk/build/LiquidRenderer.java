...
public class LiquidRenderer {
    ...
    public void tessellate(BlockState blockState, FluidState fluidState, BlockPos blockPos, TerrainBufferBuilder vertexConsumer) {
        ...
        TextureAtlasSprite[] sprites = new TextureAtlasSprite[] {
                Minecraft.getInstance().getTextureAtlas(InventoryMenu.BLOCK_ATLAS).apply(extensions.getStillTexture(fluidState, region, blockPos)),
                Minecraft.getInstance().getTextureAtlas(InventoryMenu.BLOCK_ATLAS).apply(extensions.getFlowingTexture(fluidState, region, blockPos))
        };
        ...
        if (rUf && !isFaceOccludedByState(region, Math.min(Math.min(nwHeight, swHeight), Math.min(seHeight, neHeight)), Direction.UP, blockPos, upState)) {
            ...
            float ai = 0.001F; // uvShrinkRatio fallback
